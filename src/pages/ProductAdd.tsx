...
const ProductAdd: React.FC<ProductAddProps> = () => {
  ...
  // Handle navigation to profile with wardrobe tab
  ...
  const fetchCategories = async () => {
    try {
      ...
      setError(t('productAdd.errors.fetchCategories'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const createProduct = async (images: File[]) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('productAdd.errors.createProduct'));
      }
          ...
          <div>
            <div className="relative px-6 py-3 rounded-t-lg flex items-center justify-between">
              ...
              <h2 className="absolute left-1/2 transform -translate-x-1/2 text-lg font-semibold text-gray-900">
                {navigationHistory.length === 0 ? t('productAdd.activities') : t('productAdd.types')}
              </h2>

              {/* Close Button */}
              <button 
                onClick={() => handleGoToProfile()}
                className="w-10 h-10 flex items-center justify-center rounded-full bg-white hover:bg-green-400 transition shadow-md"
              >
            ...
            <h2 className="mb-2 px-14 pt-6 font-semibold text-[20px]">
              {navigationHistory.length === 0
                ? t('productAdd.sellingForQuestion')
                : t('productAdd.sellingItemQuestion')}
            </h2>
