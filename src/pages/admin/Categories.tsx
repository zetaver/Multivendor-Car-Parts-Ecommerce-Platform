...
const Categories: React.FC<CategoriesProps> = ({ onSearch }) => {
  ...
  const searchCategories = async (query: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(t('admin.categories.errors.searchFailed'));
      }
      ...
      setError(t('admin.categories.errors.searchFailed'));
    } finally {
      setLoading(false);
      setSearching(false);
    }
  ...
  const fetchCategories = async () => {
    try {
      ...
      setError(t('admin.categories.errors.fetchFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!newCategory.image) {
        throw new Error(t('admin.categories.errors.imageRequired'));
      }
      ...
      if (!uploadResponse.ok) {
        throw new Error(uploadData.error?.message || t('admin.categories.errors.imageUploadFailed'));
      }
      ...
      if (!response.ok) {
        throw new Error(data.error?.message || t('admin.categories.errors.createFailed'));
      }

      fetchCategories();
      setFormSuccess(t('admin.categories.success.created'));
      setNewCategory({
        name: '',
        description: '',
        parentId: '',
      ...
      setFormError(err instanceof Error ? err.message : t('admin.categories.errors.createFailed'));
    } finally {
  ...
  const handleEditCategory = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error?.message || t('admin.categories.errors.updateFailed'));
      }

      // Refresh categories to get the updated hierarchy
      fetchCategories();

      setFormSuccess(t('admin.categories.success.updated'));
      ...
      setFormError(err instanceof Error ? err.message : t('admin.categories.errors.updateFailed'));
    } finally {
      setIsAdding(false);
    }
  };
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error?.message || t('admin.categories.errors.deleteFailed'));
      }

      // Refresh categories to get the updated hierarchy
      fetchCategories();

      setFormSuccess(t('admin.categories.success.deleted'));
      ...
      setFormError(err instanceof Error ? err.message : t('admin.categories.errors.deleteFailed'));
    } finally {
      setIsAdding(false);
    }
  };
    ...
    <div className="p-6">
      ...
      <div className="overflow-hidden border border-gray-200 rounded-md">
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <span className="font-medium">{t('admin.categories.table.categoryName')}</span>
            <span className="font-medium">{t('admin.categories.table.actions')}</span>
          </div>
        </div>
        ...
        <div className="divide-y divide-gray-200">
          {loading ? (
            <div className="py-4 px-6 text-center text-gray-500">
              {searching ? t('admin.categories.searching') : t('admin.categories.loading')}
            </div>
          ) : categories.length > 0 ? (
            categories.map(category => renderCategoryTree(category))
          ) : (
            <div className="py-4 px-6 text-center text-gray-500">
              {searchQuery ? t('admin.categories.noSearchResults') : t('admin.categories.noCategories')}
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">{t('admin.categories.modals.add.title')}</h2>
              <button
                onClick={() => {
                  setShowAddModal(false);
            ...
            <form onSubmit={handleAddCategory}>
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    {t('admin.categories.form.parentCategory')}
                  </label>
                  <select
                    name="parentId"
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500"
                  >
                    <option value="">{t('admin.categories.form.none')}</option>
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    ...
                    >
                      {t('admin.categories.form.chooseFile')}
                    </label>
                    {newCategory.image && (
              ...
              <div className="mt-6 flex justify-end space-x-3">
                ...
                >
                  {t('admin.categories.modals.cancel')}
                </button>
                <button
                  type="submit"
                  ...
                  {isAdding ? (
                    <div className="flex items-center">
                      <ThreeDots
                        ...
                        ariaLabel="loading"
                      />
                      <span className="ml-2">{t('admin.categories.modals.add.adding')}</span>
                    </div>
                  ) : (
                    t('admin.categories.modals.add.submit')
                  )}
                </button>
              </div>
            </form>
      ...
      {showEditModal && selectedCategory && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">{t('admin.categories.modals.edit.title')}</h2>
              <button
                onClick={() => {
                  setShowEditModal(false);
                  ...
                  {isAdding ? (
                    <div className="flex items-center">
                      <ThreeDots
                        ...
                        ariaLabel="loading"
                      />
                      <span className="ml-2">{t('admin.categories.modals.edit.updating')}</span>
                    </div>
                  ) : (
                    t('admin.categories.modals.edit.submit')
                  )}
                </button>
              </div>
            </form>
      ...
      {showDeleteModal && selectedCategory && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">{t('admin.categories.modals.delete.title')}</h2>
              <button
            ...
            <p className="mb-4">
              {t('admin.categories.modals.delete.confirmation')} <span className="font-semibold">{selectedCategory.name}</span>?
            </p>
                ...
                {isAdding ? (
                  <div className="flex items-center">
                    <ThreeDots
                      ...
                      ariaLabel="loading"
                    />
                    <span className="ml-2">{t('admin.categories.modals.delete.deleting')}</span>
                  </div>
                ) : (
                  t('admin.categories.modals.delete.submit')
                )}
              </button>
            </div>
          </div>
