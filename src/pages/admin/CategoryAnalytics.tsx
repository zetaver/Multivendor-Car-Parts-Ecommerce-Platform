  ...
  useEffect(() => {
    const fetchTopCategories = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`${t('admin.categoryAnalytics.errors.fetchFailed')}: ${response.status}`);
        }
        ...
        if (data.success) {
          ...
          throw new Error(data.message || t('admin.categoryAnalytics.errors.fetchFailed'));
        }
      } catch (err) {
        console.error('Error fetching category analytics:', err);
        setError(err instanceof Error ? err.message : t('admin.categoryAnalytics.errors.unknown'));
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(`${t('admin.categoryAnalytics.errors.productsFetchFailed')}: ${response.status}`);
        }
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold text-gray-900">{t('admin.categoryAnalytics.title')}</h1>
        ...
        <div className="flex space-x-2">
          <div className="mr-4">
            ...
            >
              <option value="sales">{t('admin.categoryAnalytics.sortBySales')}</option>
              <option value="orders">{t('admin.categoryAnalytics.sortByOrders')}</option>
            </select>
          </div>
          <button 
          ...
          >
            {t('admin.categoryAnalytics.timeRanges.week')}
          </button>
          <button 
          ...
          >
            {t('admin.categoryAnalytics.timeRanges.month')}
          </button>
          <button 
          ...
          >
            {t('admin.categoryAnalytics.timeRanges.year')}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-yellow-50 text-yellow-700 p-4 rounded-lg mb-6">
          {t('admin.categoryAnalytics.noData')}
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              {sortBy === 'orders' 
                ? t('admin.categoryAnalytics.orderDistribution') 
                : t('admin.categoryAnalytics.salesDistribution')}
            </h2>
            <div className="h-80">
                ...
                <PieChart>
                  ...
                  <Tooltip 
                    formatter={(value: number) => [
                      `${value} ${sortBy === 'orders' 
                        ? t('admin.categoryAnalytics.metrics.orders') 
                        : t('admin.categoryAnalytics.metrics.sales')}`, 
                      sortBy === 'orders' 
                        ? t('admin.categoryAnalytics.metrics.orderCount') 
                        : t('admin.categoryAnalytics.metrics.salesCount')
                    ]} 
                  />
                  <Legend />
                </PieChart>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">
              {sortBy === 'orders' 
                ? t('admin.categoryAnalytics.categoryOrderComparison') 
                : t('admin.categoryAnalytics.categorySalesComparison')}
            </h2>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm lg:col-span-2">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">{t('admin.categoryAnalytics.detailedData')}</h2>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t('admin.categoryAnalytics.table.category')}
                    </th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t('admin.categoryAnalytics.table.orderCount')}
                    </th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t('admin.categoryAnalytics.table.salesCount')}
                    </th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      {t('admin.categoryAnalytics.table.revenue')}
                    </th>
                    <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      % {t('admin.categoryAnalytics.table.ofTotal')} {sortBy === 'orders' 
                        ? t('admin.categoryAnalytics.metrics.orders') 
                        : t('admin.categoryAnalytics.metrics.sales')}
                    </th>
                  </tr>
                            ...
                            <div className="flex items-center">
                              {category.imageUrl ? (
                                ...
                                <div className="h-16 w-16 bg-gray-200 rounded-lg mr-4 flex items-center justify-center text-gray-500">
                                  {t('admin.categoryAnalytics.noImage')}
                                </div>
                              )}
                              <div>
                                ...
                                <div className="text-xs text-blue-600 mt-1">
                                  {categoryProducts.length} {t('admin.categoryAnalytics.products')} • {t('admin.categoryAnalytics.clickTo')} {isExpanded 
                                    ? t('admin.categoryAnalytics.hide') 
                                    : t('admin.categoryAnalytics.show')}
                                </div>
                              </div>
                            </div>
                          </td>
                            ...
                            <td colSpan={5} className="px-6 py-4">
                              <div className="bg-gray-50 p-4 rounded-lg">
                                <h3 className="text-sm font-medium text-gray-900 mb-3">
                                  {t('admin.categoryAnalytics.productsIn')} {category.name}
                                </h3>
                                
                                {categoryProducts.length === 0 ? (
                                  <p className="text-sm text-gray-500">{t('admin.categoryAnalytics.noProductsFound')}</p>
                                ) : (
